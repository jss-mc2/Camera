...
class PhotoCollectionViewModel: NSObject, ObservableObject {
    
    @Published var photoAssets: PhotoAssetCollectionModel = PhotoAssetCollectionModel(PHFetchResult<PHAsset>())
    ...
    init?(albumWithIdentifier identifier: String) {
        guard let assetCollection = PhotoCollectionViewModel.getAlbum(identifier: identifier) else {
    ...
    func load() async throws {
        ...
        if let smartAlbumType = smartAlbumType {
            if let assetCollection = PhotoCollectionViewModel.getSmartAlbum(subtype: smartAlbumType) {
        ...
        if let assetCollection = PhotoCollectionViewModel.getAlbum(named: name) {
        ...
        if let assetCollection = try? await PhotoCollectionViewModel.createAlbum(named: name) {
            self.assetCollection = assetCollection
            await refreshPhotoAssets()
        }
    }
    ...
    func removeAsset(_ asset: PhotoAssetModel) async throws {
        guard let assetCollection = self.assetCollection else {
            throw PhotoCollectionError.missingAssetCollection
        }
    ...
    private func refreshPhotoAssets(_ fetchResult: PHFetchResult<PHAsset>? = nil) async {
        ...
        if let newFetchResult = newFetchResult {
            await MainActor.run {
                photoAssets = PhotoAssetCollectionModel(newFetchResult)
