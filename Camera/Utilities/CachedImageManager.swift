...
actor CachedImageManager {
    ...
    func startCaching(for assets: [PhotoAssetModel], targetSize: CGSize) {
    ...
    func stopCaching(for assets: [PhotoAssetModel], targetSize: CGSize) {
        let phAssets = assets.compactMap { $0.phAsset }
        phAssets.forEach {
    ...
    @discardableResult
    func requestImage(for asset: PhotoAssetModel, targetSize: CGSize, completion: @escaping ((image: Image?, isLowerQuality: Bool)?) -> Void) -> PHImageRequestID? {
